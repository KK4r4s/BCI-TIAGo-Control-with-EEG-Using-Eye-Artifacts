...
class Classification_Algorithm:
    ...
    """
    def __init__(self, device, _save_events, queueS3_S2):
        """Initialise filter instance
        """
        print("\n------------- Classification_Algorithm Initialized --------")
        ...
        self.q_sample_sets=queue.Queue(_MAX_QUEUE_SIZE)
        self.q_Kaan_filtered_sample_sets=queue.Queue(_MAX_QUEUE_SIZE)
...
class FilterThread(threading.Thread):
    ...
    def __init__(self, main_class):
        ...
        self.q_Kaan_filtered_sample_sets=main_class.q_Kaan_filtered_sample_sets
        
        self.queueS3_S2=main_class.queueS3_S2
    ...
    def EM_B_Detection_Algorithm(self, stream, time_stream): 
        ...
        """
        # print("Entered to Algorithm, size of stream",stream.shape)
        ...
        if (np.any(Blink_max_values["peak_heights"]>156) or self.B_Flag) and np.any(time_stream):
            if not self.B_Flag:
                ...
                self.queueS3_S2.put(["Blink",self.Blinks_Rec[-1],[time_stream[0],time_stream[-1]]])
                    ...
                    try:   
                        ...
                        if self.LR_Ind_Cond or ((np.any(LR_F7_First_Max_Ind) and np.any(LR_F7_Second_Max_Ind)) and (np.any(LR_F8_First_Min_Ind) and np.any(LR_F8_Second_Min_Ind))):
                            if np.any(abs(self.Gap_LR[3,-30:])<10): #can be added to correct where it ends 
                                ...
                                self.queueS3_S2.put(["Right",[self.Chosen_Window_times[-1][0],self.Chosen_Window_times[-1][-1]],[time_stream[0],time_stream[-1]]])
                    ...
                    try:    
                        ...
                        if self.LL_Ind_Cond or ((np.any(LL_F7_First_Min_Ind) and np.any(LL_F7_Second_Min_Ind)) and (np.any(LL_F8_First_Max_Ind) and np.any(LL_F8_Second_Max_Ind))):
                            if np.any(abs(self.Gap_LL[4,-30:])<20): 
                                ...
                                self.queueS3_S2.put(["Left",[self.Chosen_Window_times[-1][0],self.Chosen_Window_times[-1][-1]],[time_stream[0],time_stream[-1]]])
    ...
    def kaan_function(self):
        ...
        while not self.q_Kaan_filtered_sample_sets.empty():
            if count==0:
                sample_list=self.q_Kaan_filtered_sample_sets.get()
                self.q_Kaan_filtered_sample_sets.task_done()
                count+=1
                
            else:
                sample_list=np.concatenate((sample_list, self.q_Kaan_filtered_sample_sets.get()), axis=1)
                self.q_Kaan_filtered_sample_sets.task_done()
                
            if self.q_Kaan_filtered_sample_sets.empty():
                """
                To check if there is any nan and set them to zero.For this algorithm to work 
                sample_list should be an array which become an array with np.concatenate
                """
                ...
                if sample_list.shape[1]==self.numb:
                    ...
                    self.EM_B_Detection_Algorithm(stream=sample_wind,time_stream=sample_time_stream) # , queueS3_S2=queueS3_S2
                    ...
                    self.q_Kaan_filtered_sample_sets.put(deepcopy(sample_list[:,self.numb:]))
                    break
                else:
                    # Inserting back elements to Queue since it is less than 100
                    # Break is requiered since q_Kaan_filtered_sample_sets is filled again
                    self.q_Kaan_filtered_sample_sets.put(deepcopy(sample_list))
                    break
    ...
    def run(self): 
        ...
        while self.sampling:
            while not self.q_sample_sets.empty():
                ...
                self.q_Kaan_filtered_sample_sets.put(deepcopy(samples))
                self.kaan_function()
