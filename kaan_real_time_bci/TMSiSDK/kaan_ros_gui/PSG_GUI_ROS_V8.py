...
#  Welcome to Move TiaGo with Eye Movements and Eye Blinks - button=="0"-------------------------
                if button=="0":
                    ...
                    if Task_event == "01" :
                        ...
                        # Robot Turn Left
                        data = bytes(json.dumps("1_Left"), encoding = 'utf-8') # Change Data Type to json
                        self.s.sendall(data) # send a message
                        ...
                        # Robot Move Forward
                        data = bytes(json.dumps("1_Forward"), encoding = 'utf-8') # Change Data Type to json
                        self.s.sendall(data) # send a message
                        ...
                        # Robot Turn Right
                        data = bytes(json.dumps("1_Right"), encoding = 'utf-8') # Change Data Type to json
                        self.s.sendall(data) # send a message
...
#  Welcome to Activate Predefined Tasks SCREEN - button=="1" -------------------       
                    ...
                    if Pressed=="Pressed":
                        if Task_event=="10":
                             data = bytes(json.dumps("2_nurse"), encoding = 'utf-8') # Change Data Type to json
                             self.s.sendall(data) # send a message

                             self.action(window, self.w_t,Task_event)
                        elif Task_event=="11":
                             data = bytes(json.dumps("2_table"), encoding = 'utf-8') # Change Data Type to json
                             self.s.sendall(data) # send a message

                             self.action(window, self.w_t,Task_event)
                        elif Task_event=="12":
                             data = bytes(json.dumps("2_scan"), encoding = 'utf-8') # Change Data Type to json
                             self.s.sendall(data) # send a message
                             
                             self.scan(window,queueS1_S2, queueS2_S1)
                        elif Task_event=="13":
                             data = bytes(json.dumps("2_dance"), encoding = 'utf-8') # Change Data Type to json
                             self.s.sendall(data) # send a message
    ...
    def scan(self,wndw_scan,queueS1_S2, queueS2_S1):
        ...
        img_path = path.split("scripts")[0]+"/images"
        img_f_name = "loading.png"
        img_info = "0"
        

        image = Image.open(img_path+"/"+img_f_name)
        ...
        while True:
            event, values = window.read(timeout=50)
            # loop untill detect 0.png then display if 
            if not os.path.exists(img_path+"/"+"0.png"):
                img_f_name = "0.png"
                while True:
                    if os.path.exists(img_path+"/"+img_f_name):
                        time.sleep(1)
                        image = Image.open(img_path+"/"+img_f_name)
                        image.thumbnail((640, 480)) # Size the Image
                        bio = io.BytesIO()
                        ...
                        number_of_objects = os.listdir(img_path)
                        N_Objects = len(number_of_objects)-1
                        break
            else:
                if not queueS2_S1.empty():
                    dist_value=queueS2_S1.get()
                    ...
                    if Scan_event=="20": # Left
                        ...
                        data = bytes(json.dumps("3_Left/"+img_info), encoding = 'utf-8') # Change Data Type to json
                        self.s.sendall(data) # send a message
                        ...
                        data = bytes(json.dumps("3_Pick/"+img_info), encoding = 'utf-8') # Change Data Type to json
                        self.s.sendall(data) # send a message
                        Pressed="Back"
                        ...
                        data = bytes(json.dumps("3_Right/"+img_info), encoding = 'utf-8') # Change Data Type to json
                        self.s.sendall(data) # send a message
                    ...
                    if Pressed=="Done" :
                        ...
                        dir_list.remove("loading.png")
                        for element in dir_list:
                            try:
                                os.remove(img_path+"/"+element)
